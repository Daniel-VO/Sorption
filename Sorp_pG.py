"""
Created 12. April 2024 by Daniel Van Opdenbosch, Technical University of Munich
...
for f in sorted(glob.glob('*.csv')):
	...
	plt.close('all')
	isotherm.plot()
	plt.tight_layout(pad=0.1);plt.savefig(filename+'_pG_iso.png')
	plt.xscale('log');plt.xlim([min(PP0)/2,2])
	plt.tight_layout(pad=0.1);plt.savefig(filename+'_pG_iso_log.png')
	...
	for i in plt.get_fignums()[0:]:
		...
		plt.figtext(0.98,0.98,filename,ha='right',va='top');plt.tight_layout(pad=0.1);plt.savefig(filename+'_pG_'+str(i-1)+'.png')
	...
	ax.legend(loc='best')
	plt.figtext(0.98,0.98,filename,ha='right',va='top');plt.tight_layout(pad=0.1);plt.savefig(filename+'_pG_PSD.png')

	plt.close('all')
	model_iso=pgm.model_iso(isotherm,model=['Henry','Langmuir','bet'],verbose=True)
	models.append(model_iso.model)
	plt.figtext(0.98,0.98,filename,ha='right',va='top');plt.tight_layout(pad=0.1);plt.savefig(filename+'_pG_mod.png')

for i,(f,b,l,t) in enumerate(results):
	print(f+' BET: '+f"{b['area']:.2f}",end=' ',file=open('results.log','a'))
	print('Langmuir:'+f"{l['area']:.2f}",end=' ',file=open('results.log','a'))
	# ~ print('t:'+f"{t['results'][0]['area']:.2f}",end=' ',file=open('results.log','a'))
	print('\n',models[i],file=open('results.log','a'))
